...
void write_server(int client, char *message) {
    ...
    bytes_sent = write(client, messageArr, strlen(messageArr)+1);
    if (bytes_sent > 0) {
        printf("Sent : \n%s\t (%d bytes)\n", messageArr, bytes_sent);
    }
    printf("write_server done!\n");
}
...
char *getPEPProfile(){
    ...
    infile = fopen("./resources/profile/pep.profile", "r");
    if(infile == NULL)
        return -1;
...
int handleMessage(int client, char *recv_message){

    char *ret_message;

    if(!strncmp("close", recv_message, 5)) {
        printf("[CLOSE CONNECTION]\n");
        return -1;
    } else if(!strncmp("profile", recv_message, 7)){
        printf("[SEND PEP PROFILE TO CLIENT]\n");
        ret_message = getPEPProfile();
        if ( ret_message == -1) {
            printf("error occured during read profile file.\n");
			write_server(client, "{\"error\":\"No PEP Profile\"}");
			free(ret_message);
            return -1; //continue;
        }
    }

    write_server(client, ret_message);
    free(ret_message);
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            if ( recv_message == NULL ){
                ...
                int flag = handleMessage(client, recv_message);
                if(flag == -1) break;
            }
